        ...
        <nav aria-label="...">
            <ul class="pagination mb-0" id="tablePagination">
                <li class="page-item <?= (int) $pagination['page'] <= 1 ? 'disabled' : '' ?>">
                    <a class="page-link" href="<?= htmlspecialchars($buildAccountExecutiveUrl(['page' => max(1, (int) $pagination['page'] - 1)]), ENT_QUOTES, 'UTF-8') ?>">Previous</a>
                </li>
                <?php for ($pageNum = $startPage; $pageNum <= $endPage; $pageNum++): ?>
                <li class="page-item <?= (int) $pagination['page'] === $pageNum ? 'active' : '' ?>">
                    <a class="page-link" href="<?= htmlspecialchars($buildAccountExecutiveUrl(['page' => $pageNum]), ENT_QUOTES, 'UTF-8') ?>" <?= (int) $pagination['page'] === $pageNum ? 'aria-current="page"' : '' ?>><?= $pageNum ?></a>
                </li>
                <?php endfor; ?>
                <li class="page-item <?= (int) $pagination['page'] >= (int) $pagination['totalPages'] ? 'disabled' : '' ?>">
                    <a class="page-link" href="<?= htmlspecialchars($buildAccountExecutiveUrl(['page' => min((int) $pagination['totalPages'], (int) $pagination['page'] + 1)]), ENT_QUOTES, 'UTF-8') ?>">Next</a>
                </li>
            </ul>
        </nav>
    </div>
